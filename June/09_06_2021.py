...
class Solution:
    def helper(self, nums, start_pivot, end_tether, k, rolling_sums=[0]):
        print("rollingsums", rolling_sums)
        if start_pivot >= end_tether:
            return max(rolling_sums)
        j = 0
        new_rolling_sums = []
        print("s", start_pivot, "e", end_tether)
        while j < k and start_pivot + j < end_tether:
            print(">>", j)
            if len(rolling_sums) == 1:
                prev_sum = rolling_sums[0]
            else:
                prev_sum = rolling_sums[j]
            new_rolling_sums.append(nums[start_pivot + j] + prev_sum)
            j += 1
        return self.helper(nums, start_pivot + k, end_tether, k, new_rolling_sums)
    ...
    def maxResult(self, nums: List[int], k: int) -> int:
        ...
        while i < len(nums):
            ...
            if (num > 0) or (i == len(nums) - 1):
                ans += num
                if fast - slow > 1 and fast - slow > k:
                    print(f"calling helper slow+1 {slow+1}, fast {fast}, k {k}, [ans] {[ans]}")
                    ans = self.helper(nums, slow+1, fast, k, [ans])
                    print("ans helper", [ans])
                slow = i
            i += 1
        return ans
